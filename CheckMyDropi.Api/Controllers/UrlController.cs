...
namespace CheckMyDropi.Api.Controllers
{
    ...
    {
        private static readonly string BASEURL = "https://raw.githubusercontent.com/littl3field/DodgyDomainsBot/master/";
        ...
        {
            return UpdateFunction(@"COVID-Dodgy-Domains.txt");
        }

        [HttpGet]
        [Route("Updatefrom/{url}")]
        public IActionResult UpdateFrom(string url)
        {
            return UpdateFunction(url);

        }
       

        private IActionResult UpdateFunction(string url)
        {
            _logger.LogInformation("{0} - Updating database from {2} {1}", DateTime.Now, HttpContext.TraceIdentifier, url);
